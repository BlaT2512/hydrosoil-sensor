...
private:
    // Persistent cache for offline time tracking and failed request tracking
    struct RtcCache {
        uint32_t checksum; // Checksum to verify no corrupted data from the RAM
        uint32_t clock; // (s) Accumulated time during outage
        uint32_t lastSleep; // (us) Duration of last sleep
        uint8_t failedRequests; // Backoff counter for outage
    };
    ...
    String collectSensorData(); // Reads sensors and outputs data in JSON foramt, ready for appending to POST body
    ...
    String readBuffer(); // Read and output any lines in the buffer in JSON array format, for appending to POST body
